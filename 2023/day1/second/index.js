...
const replaceWordWithDigit = (word, type) => {
    if (type === 'reverse') {
        let smallestPositiveValue = Infinity;
        let fistKey = null;
        const indexMap = new Map()
        let toFind = [...reverseDigitMap.keys()];

        // Find first occurrence of map keys:
        toFind.forEach(value => {
            indexMap.set(value, word.indexOf(value))
        })
        indexMap.forEach((value, key) => {
            if (value >= 0 && value < smallestPositiveValue) {
                smallestPositiveValue = value
                fistKey = key;
            }
        })

        return word.replace(fistKey, reverseDigitMap.get(fistKey))
    } else {
        let smallestPositiveValue = Infinity;
        let fistKey = null;
        const indexMap = new Map()
        let toFind = [...digitMap.keys()];
        ...
        return word.replace(fistKey, digitMap.get(fistKey))
    }

}

const reverseString = (word) => word.split('').reverse().join('');
const extractDigitFromWord = (word) => word.match(/\d/)[0];

const sum = input.map(
    word => {
        const firstDigit = extractDigitFromWord(replaceWordWithDigit(word));
        const lastDigit = extractDigitFromWord(replaceWordWithDigit(reverseString(word), 'reverse'));
