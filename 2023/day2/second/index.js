        ...
        const values = game
            ...
            .map(
                value => {
                    ...
                    if(check[1] === 'red') {
                        if (Number(check[0]) > maxRed) {
                            maxRed = Number(check[0])
                        }
                    } else if(check[1] === 'blue') {
                        if (Number(check[0]) > maxBlue) {
                            maxBlue = Number(check[0])
                        }
                    } else if(check[1] === 'green'){
                        if (Number(check[0]) > maxGreen) {
                            maxGreen = Number(check[0])
                        }
                    }
                }
            )
        power.push(maxRed*maxGreen*maxBlue)

    }
)

const sum = power.reduce((accumulator, current) => accumulator + current, 0)
console.log(sum)