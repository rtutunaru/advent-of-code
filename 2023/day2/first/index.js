        ...
        const values = game
            ...
            .map(
                value => {
                    ...
                    if(check[1] === 'red') {
                        return Number(check[0]) <= MAX_RED;
                    } else if(check[1] === 'blue') {
                        return Number(check[0]) <= MAX_BLUE;
                    } else if(check[1] === 'green'){
                        return Number(check[0]) <= MAX_GREEN;
                    }
                }
            )
            .every(value => value === true)
        console.log(values)
        if (values) gameIds.push(Number(id))
    }
)

const sum = gameIds.reduce((accumulator, current) => accumulator + current, 0)
console.log(sum)