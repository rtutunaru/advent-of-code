...
matrix.forEach((row, i) => {
    ...
    row.forEach((currentCharacter, j) => {
        if (!isNaN(Number(currentCharacter))) {
            number += currentCharacter;

            for (let k = 0; k < 8; k++) {
                const nRow = i + NEIGHBOURS[k];
                const nCol = j + NEIGHBOURS[k + 1];
                if (nRow >= 0 && nCol >= 0 && nRow < matrix.length && nCol < matrix[0].length) {
                    if (matrix[nRow][nCol] === GEAR_SYMBOL) {
                        key = `${nRow}:${nCol}`;
                        hasSymbol = true;
                        break;
                    }
            ...
            if (j === matrixSize - 1 && number && hasSymbol) {
                processNumber(number, key);
            }
        } else {
            if (number && hasSymbol) {
                processNumber(number, key);
    ...
    function processNumber(numStr, symbolIdx) {
        const parsedNum = Number(numStr);
        if (ratios.has(symbolIdx)) {
            sum += ratios.get(symbolIdx) * parsedNum;
        } else {
            ratios.set(symbolIdx, parsedNum);
        }
    }
});
